...
charging_number_sf <- charging_sf %>%
  mutate(state_abb = state) %>%
  mutate(state = state.name[match(state, state.abb)]) %>%
  mutate(state = ifelse(state_abb == "DC", "District of Columbia", state))

charging_ev <- left_join(x = charging_number_sf, y = ev_numbers, by = "state") %>%
 drop_na()
```
